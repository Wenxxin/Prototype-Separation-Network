...
def main(args):
    ...
    query_img = [F.to_tensor(Image.open("/public/home/G19940018/FeatureGroup/zhangwenxin/SeqNet-master/demo_imgs/query.jpg").convert("RGB")).to(device)]
    query_target = [{"boxes": torch.tensor([[0, 0, 466, 943]]).to(device)}]
    query_feat = model(query_img, query_target)[0]

    gallery_img_paths = sorted(glob("/public/home/G19940018/FeatureGroup/zhangwenxin/SeqNet-master/demo_imgs/gallery-*.jpg"))
    for gallery_img_path in gallery_img_paths:
        print("Processing {gallery_img_path}")
        ...
        # data = gallery_output.tolist()
        # pickle.dump(gallery_output, open("/public/home/G19940018/FeatureGroup/zhangwenxin/GLCNet-main/demo_imgs/result/myclass.pkl", 'wb'))  # 序列化
        # write_json(gallery_output, "/public/home/G19940018/FeatureGroup/zhangwenxin/GLCNet-main/demo_imgs/result/results.json")
        #cuda:1
