...
from models.attentionnet import attention,mapaa,apool
...
class SeqNet(nn.Module):
    def __init__(self, cfg):
        ...
        anchor_generator = AnchorGenerator(
            ...
            # sizes=((32, 64, 128, 256, 512),), aspect_ratios=((0.5, 1.0, 2.0),)
            #kuanggaobi,((0.9, 1.4, 1.9, 2.3, 2.5, 2.7, 2.9, 3.1, 3.5, 4.0, 4.6, 5.2),)
            #五种类型的边界框
        )
        head = RPNHead(
        ...
        roi_heads = SeqRoIHeads(
            ...
            detectionscore=cfg.MODEL.LOSS.OIM_SCALAR,
            # SeqNet
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        if targets is not None and not query_img_as_gallery:
            ...
            # feat = box_features["feat_res5"]
            feat, map = self.roi_heads.attention(box_features["feat_res5"])
            # box_features["feat_res5"] = F.adaptive_max_pool2d(feat, 1)
            box_features["feat_res5"] = self.roi_heads.adapool(feat, 1)
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        _, detector_losses = self.roi_heads(features, proposals, images.image_sizes, targets)#seq两部分
        ...
        losses["loss_map"] *= self.lw_box_reid
        # losses["loss_part_body"] *= self.lw_box_reid
        return losses
...
class SeqRoIHeads(RoIHeads):
    def __init__(
        ...
        **kwargs
        ...
        self.reid_loss = OIMLoss(256, num_pids, num_cq_size, oim_momentum, oim_scalar, detectionscore)
        self.reidpart_loss = hardLoss()
        ...
        self.maploss = mapaa()
        # self.adapool = F.adaptive_max_pool2d()
        self.adapool = apool()
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        feat = proposal_features["feat_res5"]
        proposal_features["feat_res5"] = F.adaptive_max_pool2d(feat, 1)
        ...
        if self.training:
            ...
            boxes, mis, box_pid_labels, box_reg_targets = self.select_training_samples(boxes, targets)
        ...
        if not self.training and query_img_as_gallery:
            ...
            # feat = gt_box_features["feat_res5"]
            # part_body = self.part(feat)
            feat,map = self.attention(gt_box_features["feat_res5"])
            # gt_box_features["feat_res5"] = F.adaptive_max_pool2d(feat, 1)
            gt_box_features["feat_res5"] = self.adapool(feat, 1)
        ...
        box_features["feat_res5"], map = self.attention(box_features["feat_res5"])
        mfeature = box_features["feat_res5"]

        # box_features["feat_res5"] = F.adaptive_max_pool2d(box_features["feat_res5"], 1)
        box_features["feat_res5"] = self.adapool(box_features["feat_res5"], 1)
        ...
        if self.training:
            ...
            # box_labels = [y.clamp(0, 483) for y in box_pid_labels]
            # print(box_labels)
            ...
            gtx = [t["boxes"] for t in targets]
            gtlabel = [t["labels"] for t in targets]
            IOU, score = computeiou1(
                boxes,
                box_pid_labels,
                gtx,
                gtlabel,mis)
            ...
            aa = box_cls_scores.view(-1, 1)
            box_embeddings_rs = torch.sigmoid(aa)
            loss_box_reid,lutt = self.reid_loss(box_embeddings, box_pid_labels,IOU)#box_embeddings_rs
            loss_map = self.maploss(mfeature,map,lutt,box_pid_labels)
            losses.update(loss_box_reid=loss_box_reid)
            losses.update(loss_map=loss_map)
        else:
...
class NormAwareEmbedding(nn.Module):
    ...
    def __init__(self, featmap_names=["feat_res4", "feat_res5"], in_channels=[1024, 2048], dim=256):
        ...
        for ftname, in_channel, indv_dim in zip(self.featmap_names, self.in_channels, indv_dims):
            # proj = nn.Sequential(nn.Linear(in_channel, indv_dim), PrototypeNorm1d(indv_dim))#nn.BatchNorm1d(indv_dim)
            proj = nn.Sequential(nn.Linear(in_channel, indv_dim), nn.BatchNorm1d(indv_dim))#nn.BatchNorm1d(indv_dim)
...
class BBoxRegressor(nn.Module):#huigui
    """
...
def box_diou(b1, b2):
    ...
    b1_xy = b1_mins+(b1_wh/2)
    # 求出真实框左上角右下角
    b2_mins = b2[..., :2]
    b2_maxes = b2[..., 2:4]
    b2_wh = b2_maxes - b2_mins
    b2_xy = b2_mins+(b2_wh/2)
    # 求真实框和预测框所有的iou
...
def computeiou1(
    ...
    for id in range(num_images):
        indx = torch.nonzero(box_labels[id] > 0).squeeze(1)
        gtbox.append(gtx[id][mis[id][indx]])

    box_labels = torch.cat(box_labels)
    box = torch.cat(boxes)
    gtbox = torch.cat(gtbox)

    sampled_pos_inds_subset = torch.nonzero(box_labels > 0).squeeze(1)
    key = box[sampled_pos_inds_subset]

    lossdiou,score = box_diou(key, gtbox)

    # lossiou = torch.sum(lossdiou)
    # loss_box_reg = lossdiou / box_labels.numel()
    return lossdiou,score


def compute(boxes,box_labels):#只算大小
    box_labels = torch.cat(box_labels)
    box = torch.cat(boxes)
    sampled_pos_inds_subset = torch.nonzero(box_labels > 0).squeeze(1)
    key = box[sampled_pos_inds_subset]
    b1_wh = key[..., 2:4]  # wh
    score = b1_wh[..., 0] * b1_wh[..., 1]
    min = score.min()
    max = score.max()
    ds = max-min
    score = (score-min).true_divide(ds)
...
def computeiou(
    ...
    lossdiou = 1 - box_diou(box_regs[sampled_pos_inds_subset, labels_pos], box_reg_targets[sampled_pos_inds_subset])
    lossiou = torch.sum(lossdiou)
    lossdiou = lossiou / box_labels.numel()
    return lossdiou
